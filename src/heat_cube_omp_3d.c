...
void compute_averages(const SimulationConfig *cfg, const float *u,
    ...
    #pragma omp parallel for collapse(3) reduction(+:sum_total, sum_cube) schedule(static)
    for (int k = 0; k < cfg->nz; k++) {
        for (int j = 0; j < cfg->ny; j++) {
            for (int i = 0; i < cfg->nx; i++) {
                float z = (k + 0.5f) * cfg->dz;
...
int main(int argc, char *argv[]) {
    ...
    int n_cube = 0;
    #pragma omp parallel for collapse(3) reduction(+:n_cube) schedule(static)
    for (int k = 0; k < cfg.nz; k++) {
        for (int j = 0; j < cfg.ny; j++) {
            for (int i = 0; i < cfg.nx; i++) {
                float z = (k + 0.5f) * cfg.dz;
    ...
    printf("========================================================\n");
    printf("  3D Heat Transfer Solver - OpenMP CPU Parallel\n");
    printf("========================================================\n");
    ...
    printf("OpenMP Threads: %d (configured: %d)\n", omp_get_max_threads(), cfg.num_threads);
    ...
    char cfg_label[64];
    snprintf(cfg_label, sizeof(cfg_label), "%d threads", omp_get_max_threads());
    print_performance("OpenMP CPU", cfg_label, total_cells, cfg.nsteps, elapsed_sec);
